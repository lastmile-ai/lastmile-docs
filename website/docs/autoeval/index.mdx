...
It solves a key problem in generative AI application development -- developing metrics that accurately represent your application's performance, which you can
use to iteratively drive improvements and have confidence in production.
...
AutoEval starts from the application's trace data, helps label the interactions using LLM Judge where you can specify an evaluation criteria in natural language (prompt),
and then fine-tune an alBERTa SLM to learn the distribution of those labeled interactions. You can then use the fine-tuned model
as an eval, or at runtime as a guardrail. For e.g. if a response is deemed to have hallucinated, i.e. unsufficiently faithful to the data, then deny that response from reaching the user.
...
### Online inference
You can use the evaluator models for offline tasks such as running evals, as well as online tasks such as guardrails.
