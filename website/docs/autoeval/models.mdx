...
AutoEval ships with small language models (SLMs) for evaluation tasks. The [Metrics section](/autoeval/metrics) lists all out-of-the-box metrics available on the platform.

In addition, if you need to design your own metric, you can [fine-tune](/autoeval/fine-tune) LastMile's alBERTa 🍁 base model to have a custom evaluator.
...
:::info
Since alBERTa 🍁 models are small and fast, you can run them **online** as guardrails. Simply use the API in application code directly.
:::